...
namespace WebAppMain.Controllers
{
    public class UserController : Controller
    {
        ...
        public UserController(UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager, ApplicationDbContext applicationDbContext)
        {
        ...
        public async Task<IActionResult> IndexAsync(string searchString, int? page, string currentFilter, string className)
        {
            ...
            // фильтруем пользователей по классу, только если текущий пользователь - учитель
            if (user != null && _userManager.IsInRoleAsync(user, "Учитель").Result)
            {
                users = users.Where(u => u.Class != null && u.Class.Id_Class == user.Id_Class).ToList();
            }
